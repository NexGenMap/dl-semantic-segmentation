	...
	with tf.variable_scope("layer{}".format(name)):
			for i, F in enumerate(n_filters):
					...
					net = tf.layers.conv2d(
							...
							activation=None,
							padding='valid',
							kernel_regularizer=tf.contrib.layers.l2_regularizer(params['l2_regularizer']),
...
def upconv_concat(upconv_input, concat_input, n_filter, mode, params, name, padding="valid"):
...
def description(features, labels, mode, params, config):
	...
	dropout = tf.layers.dropout(conv9, name="dropout", rate=params['dropout_rate'], training=(mode == tf.estimator.ModeKeys.TRAIN))

	output = tf.layers.conv2d(dropout, 1, (1, 1), name='output', activation=tf.nn.sigmoid, padding='valid',
	...
	with tf.name_scope("img_metrics"):
		...
		for i in range(3):
			input_data_viz.append(rescale_data_viz(input_data[:,:,:,i]));

		input_data_viz = tf.image.convert_image_dtype( tf.stack(input_data_viz, axis=3), tf.uint8, saturate=True)

		output_viz = tf.image.grayscale_to_rgb(output[:,:,:,:])
		labels_viz = tf.image.grayscale_to_rgb(labels[:,:,:,:])
	
		tf.summary.image('img',  input_data_viz, max_outputs=params['tensorboard_maxoutput'])
		tf.summary.image('output', output_viz, max_outputs=params['tensorboard_maxoutput'])
		tf.summary.image('labels', labels_viz, max_outputs=params['tensorboard_maxoutput'])
